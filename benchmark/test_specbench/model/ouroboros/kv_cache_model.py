...
class KVCacheModelSimpleWithGuess():
    ...
    def _forward_with_kvcache(self, input_ids : torch.Tensor, guess, use_debug = False) -> torch.Tensor:
        ...
        if self._past_key_values is None:
            ...
            self._history = outputs.logits.argmax(dim=-1)
            ...
            ncq_t = not_cached_q.argmax(dim=-1)
            # self._prob_history = torch.cat([self._prob_history, not_cached_q], dim=1)
            self._history = torch.cat([self._history, ncq_t], dim=1)
    ...
    def rollback(self, end_pos : int):
        ...
        self._history = self._history[:, :end_pos]
        self.idx = end_pos
    ...
    def confirm(self, cur_len, start_pos, length):
        ...
        self._history[:, cur_len:cur_len + length] = self._history[:, start_pos: start_pos + length]
        self._history = self._history[:, :cur_len + length]
