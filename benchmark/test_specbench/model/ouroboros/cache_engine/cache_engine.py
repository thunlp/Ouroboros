...
class CacheEngine():
    ...
    def insert_simple(self, lst_token, tup): # tup = ngram
        ...
        elif len(self.token_map[lst_token]) < self.guess_set_size:
            self.token_map[lst_token].append(tup) 
        else:
            assert len(self.token_map[lst_token]) == self.guess_set_size
    ...
    def insert(self, lst_token, new_results, past_tokens, GUESS_SET_SIZE, LEVEL, WINDOW_SIZE):
        if GUESS_SET_SIZE != -1:
            ...
            for i in range(1, WINDOW_SIZE):
                ...
                elif len(self.token_map[past_tokens[0][i - 1]]) < self.guess_set_size:
                    self.token_map[past_tokens[0][i - 1]].append(tup) 
                else:
                    assert len(self.token_map[past_tokens[0][i - 1]]) == self.guess_set_size
