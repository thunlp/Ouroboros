...
def ouroboros_specbench(inputs, model, tokenizer, max_new_tokens, max_steps=512, lookahead_level=5, guess_set_size=15, window_size=15, gamma=6, topk=3, draft_model=None, ngram_cache=None):
    eos_token_id = tokenizer.eos_token_id
    prefix = inputs.input_ids
    if ngram_cache is None:
        ngram_cache = CacheEngine(lookahead_level, guess_set_size)
    init_len = prefix.shape[-1]

    guess_size = lookahead_level - 1
    
    approx_model_cache = KVCacheModelLade(draft_model, window_size=window_size, guess_set_size=guess_set_size, lookahead_level=lookahead_level, topk=topk)
    target_model_cache = KVCacheModelSimpleWithGuess(model, lookahead_level=lookahead_level)
    ...
    for idx in range(max_steps):
        ...
        for i in range(gen_len):
            ...
            # t_tok = target_model_cache._prob_history[:, prefix_len + i - 1, :].argmax(dim=-1, keepdim=True).to(j.device)
            t_tok = target_model_cache._history[:, prefix_len + i - 1].to(j.device)
            if t_tok != j:
                n = prefix_len + i - 1
                break
            if j == eos_token_id:
        ...
        if n < prefix_len + gen_len - 1:
            ...
            sep_pos = prefix_len + gen_len
            # refined = target_model_cache._prob_history[0, n:sep_pos, :].argmax(dim=-1).tolist()
            ...
            # find the longest guess
            guess_num = len(guess) // guess_size
            first_tok = int(target_model_cache._history[:, n])
            # first_tok = int(target_model_cache._prob_history[:, n, :].argmax(dim=-1))
            beg_pos = prefix_len + gen_len
            candidate = [first_tok]
            longest_can_len = 1
            ...
            for i in range(guess_num):
                # real_ngram = [first_tok] + target_model_cache._prob_history[0, beg_pos + i * guess_size : beg_pos + i * guess_size + guess_size, :].argmax(dim=-1).tolist()
                real_ngram = [first_tok] + target_model_cache._history[0, beg_pos + i * guess_size : beg_pos + i * guess_size + guess_size].tolist()
                pred_ngram = guess[i * guess_size : (i + 1) * guess_size]
                ml = 0
                for j in range(guess_size):
                    ml = j
        ...
        accepted_length_list.append(prefix.shape[-1] - prefix_len)

        if end_pos is not None:
            break
        if prefix.shape[1] - init_len > max_new_tokens:
            break
        if prefix.shape[1] > 1960:
            break
        

    return prefix, prefix.shape[1] - init_len, idx + 1, accepted_length_list
